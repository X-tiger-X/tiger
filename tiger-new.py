	...
	try:
		...
		while True:
			...
			if rom in(""):
				...
				jalane("\n%s!%s PLEASE PASTE COOKIE FB"%(M,M))
...
class pilihan:
	...
	def MassalGRAPH(self, token):
		...
		try:
			print ('')
			jalane(f"\r{M}{til}{O} Mengumpulkan Id {M}> {M}[{H}{len(self.id)}{M}] ",end="")
		except:
			pass
	...
	def PublikGRAPH(self, user, token):
		try:
			...
			for i in po['friends']['data']:
				self.id.append(f"{i['id']}<=>{i['name']}")
				print(f"\r{M}{til}{O} Mengumpulkan Id {M}> {M}[{H}{len(self.id)}{M}] ",end="")
		except:
			pass
...
class Crack:
	...
	def romiy(self, id):
		...
		if unikers in ('Y', 'y'):
			...
			print ('\n%s•%s [ %sHalbzhera [!]%s ]\n'%(U,O,U,O))
			print ('%s• %s01%s CRACK %s(FAST)'%(U,P,O,P,M))
			self.langsung()
		else:
